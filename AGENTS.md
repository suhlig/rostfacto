...
- Besides databases, an interrupted run can leave headless Firefox sessions (temp `rust_mozprofile` profiles) and geckodriver processes behind; they pile up, slow the suite down, and make it flaky. Clear them with `scripts/cleanup-tests.sh` (or `pkill -f rust_mozprofile`).
